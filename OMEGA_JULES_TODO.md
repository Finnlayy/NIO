...
> **STATUS (2026-09-07): Phases 1–5 COMPLETE.** All deliverables implemented;
> `cd Architect && make test` → 214 tests OK (4 environment-conditional skips).
> Divergences for Phases 4–5 are documented in `Architect/docs/DIVERGENCES.md`
> (stdlib preference model instead of ONNX; QPainter trajectory timeline).
...
### Documentation
- [x] `[P1]` `Architect/README.md` — Create. Must include: directory map (`core/`, `limbs/`, `gui/`, `models/`), installation (`pip install -r requirements.txt`), phase status table, divergence notes, how to run (`python -m gui.app` or `make run`), how to test (`python test_init.py`).
- [x] `[P1]` `Architect/docs/ARCHITECTURE.md` — Create. Describe: dependency model (PyQt6, torch, onnx, qdrant, ccxt.pro, google-genai, numpy, pydantic-settings), transport model (external APIs + WebSocket feeds vs. blueprint file transport), data flow (`daemon → GUI`, `limb → workspace`), directory contract.
- [x] `[P1]` `Architect/docs/PHASES.md` — Create. 5-phase table: phase number, status, deliverables, dependencies, blockers, estimated timeline.
- [x] `[P1]` `Architect/docs/DIVERGENCES.md` — Create. Document every intentional divergence from blueprint: dependency model (external services vs. stdlib-only), transport (APIs vs. file-based), domain (trading vs. generic orchestration), UI (PyQt6 vs. React/Next.js), protocol (planned vs. 492-line spec).
...
- [x] `[P1]` `Architect/Makefile` — Add `test` target (`python test_init.py`) and `lint` target (`ruff check .` or `flake8 .` if available).
- [x] `[P1]` `.gitignore` — Verify entries: `runtime/`, `workspace/`, `.env`, `data/`, `__pycache__/`, `.test-build/`, `*.pyc`, `*.log`.
...
- [x] `[P2]` `Architect/limbs/intelligence/genai_client.py` — Line 32 (`generate_journal`): Replace `return "Stub journal entry."` with real Gemini SDK call: `self.client.models.generate_content(model=self.model_deep, contents=...)`. Must handle `api_key` empty (raise or log warning). Must save output to `Architect/runtime/reflections/<timestamp>_journal.json`.
- [x] `[P2]` `Architect/limbs/intelligence/genai_client.py` — Line 36 (`reflect_dsr`): Replace `return "Stub DSR reflection."` with structured output: JSON or dict with fields (`diagnosis`, `root_cause`, `recommended_action`, `confidence`). Must use `post_mortem_data` input (dict or JSON string). Must handle API errors gracefully (`try/except` around SDK call → return `{"error": "API unavailable", "fallback": "manual_review_required"}`).
- [x] `[P2]` `Architect/limbs/intelligence/genai_client.py` — Add `__all__`, docstrings, type hints (`context: dict`, `post_mortem_data: dict` → `str` or `dict`).

### Memory-Driven Planning
- [x] `[P2]` `Architect/limbs/intelligence/memory_planner.py` — Create. Must implement:
...
- [x] `[P2]` `Architect/core/planning_config.py` — Create (optional): planning thresholds (`min_confidence_for_dispatch`, `max_steps_per_plan`, `agent_capabilities_mapping`).

### Agent Assignment
- [x] `[P2]` `Architect/core/agent_assignment.py` — Create. Must implement:
...
- [x] `[P2]` `Architect/core/state_machine.py` — Add transition rules. Must implement:
...
- [x] `[P2]` `Architect/core/state_machine.py` — Add guard conditions: `plan_approved` requires `confidence >= min_confidence_for_dispatch` (use `core/config.py` value or default `0.7`); `execution_started` requires `feed_live == True` (`TheJudgeM8` Axiom 1).

### Post-Mortem Triggers
- [x] `[P2]` `Architect/core/post_mortem_trigger.py` — Create. Must implement:
...
### Baseline Retrieval
- [x] `[P2]` `Architect/core/baseline_retriever.py` — Create. Must implement:
...
- [x] `[P2]` `Architect/runtime/baselines/` — Create directory (add to `.gitignore`? No — baselines should persist but not be versioned. Add `.gitignore` entry: `runtime/baselines/*.json`? Or keep them tracked? Decision: add `.gitignore` for `runtime/` and document in `README.md`. Baselines are runtime artifacts, not source code.)

### Phase 2 Tests
- [x] `[P2]` `tests/test_memory_planner.py` — Create. Must cover: `create_plan`, `update_plan`, `get_plan`, Qdrant integration mock.
- [x] `[P2]` `tests/test_agent_assignment.py` — Create. Must cover: mapping logic, capability registry.
- [x] `[P2]` `tests/test_state_machine_transitions.py` — Create. Must cover: all valid transitions, guard conditions, invalid transitions blocked.
- [x] `[P2]` `tests/test_post_mortem_trigger.py` — Create. Must cover: trigger conditions, event emission, `GenAIClient` integration mock.
- [x] `[P2]` `tests/test_baseline_retriever.py` — Create. Must cover: load, compare, missing baseline handling.
...
### Manifest System
- [x] `[P3]` `Architect/core/manifest.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/schemas/manifest.schema.json` — Create. JSON Schema (2020-12). Fields: `manifest_version` (string, enum: [`1.0`]), `author` (string), `timestamp` (ISO8601), `changes` (array of objects: `{file_path, change_type, line_numbers, checksum_before, checksum_after}`), `safety_flags` (object: `{core_change, dependency_change, model_update}`), `approval_level` (string: [`system`, `human`]).

### Manifest Validation (`validateManifestUpdate`)
- [x] `[P3]` `Architect/core/manifest_validation.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/core/self_modification_safety.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/limbs/bootstrap_limb.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/core/policy.py` — Create (or extend from blueprint design). Must implement:
...
- [x] `[P3]` `tests/test_ouroboros.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/core/events.py` — Create. Must implement:
...
- [x] `[P3]` `Architect/runtime/system.log` — Create directory/file? No — file created by event bus at first emission. Add `.gitignore`: `runtime/system.log`, `runtime/*.log`.
- [x] `[P3]` `tests/test_event_bus.py` — Create. Must cover: NDJSON format, whitelist validation (invalid kind rejected), `clock_s` coherence (same `t0` referenced), emission to stderr/file, event structure validation.

### Archive / Ledger
- [x] `[P3]` `Architect/core/archive.py` — Create. Must implement:
...
- [x] `[P3]` `tests/test_archive.py` — Create. Must cover: save/lookup, digest verification, stats, verify failures, compact operation.

### Timer System
- [x] `[P3]` `Architect/core/timer.py` — Create. Must implement:
...
- [x] `[P3]` `tests/test_timer.py` — Create. Must cover: `deadline` mode (remaining time decreases, timeout triggered), `unlimited` mode (`elapsed_s` increases, `remaining_ms` is `null`), `safety_net_s` activation (escalation event emitted), persistence (save/load preserves state), clock coherence (`t0` same across all references).
...
- [x] `[P4]` `Architect/schemas/outcome.schema.json` — Create. Fields: `job_id` (string), `intent_id` (string), `limb` (string), `result` (object: `{verdict, errors, metrics}`), `verdict` (string: [`accept`, `reject`, `escalate`, `timeout`]), `errors` (array of strings), `metrics` (object), `elapsed_s` (float), `clock_s` (float), `timestamp` (ISO8601).
- [x] `[P4]` `Architect/core/outcome_parser.py` — Create. Implement `OutcomeParser.parse(file_path)` — read result file (JSON); validate against `outcome.schema.json`; return `Outcome` object or raise `SchemaInvalid`.

### Feedback Processing
- [x] `[P4]` `Architect/schemas/feedback.schema.json` — Create. Fields: `outcome_id` (string), `rater_id` (string), `rating` (int, 1–5), `preference` (object: `{chosen: string, rejected: string}` or `null`), `comment` (string), `timestamp` (ISO8601).
- [x] `[P4]` `Architect/core/feedback_processor.py` — Create. Implement:
...
- [x] `[P4]` `Architect/core/learning/skill_profiles.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_skill_profiles.py` — Create. Must cover: update logic, Wilson score computation, persistence.

### SM-2 Spaced Repetition
- [x] `[P4]` `Architect/core/learning/spaced_repetition.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_spaced_repetition.py` — Create. Must cover: SM-2 algorithm correctness (compare with reference), schedule computation, persistence.

### Error Pattern Detection
- [x] `[P4]` `Architect/core/learning/error_patterns.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_error_patterns.py` — Create. Must cover: detection logic, frequency counting, alert emission.

### Knowledge Base
- [x] `[P4]` `Architect/core/learning/knowledge_base.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_knowledge_base.py` — Create. Must cover: save/search, extraction logic.

### Risk Guards (Learning Engine Safety)
- [x] `[P4]` `Architect/core/learning/risk_guards.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_risk_guards.py` — Create. Must cover: guard checks against invariants, storage, alert emission.

### `defineEval` Pipeline
- [x] `[P4]` `Architect/core/evaluation/define_eval.py` — Create. Implement:
...
- [x] `[P4]` `Architect/core/evaluation/evaluators.py` — Create (optional): reference evaluators (`correctness_evaluator`, `safety_evaluator`, `consistency_evaluator`).
- [x] `[P4]` `tests/test_define_eval.py` — Create. Must cover: `define_eval` interface, `EvalPipeline.run()`, result format, audit trail.

### Feedback UI (PyQt6)
- [x] `[P4]` `Architect/gui/views/feedback_dialog.py` — Create. Must implement:
...
- [x] `[P4]` `tests/test_feedback_dialog.py` — Create (optional, mock PyQt6): test submit logic, input validation, file output format.

### Preference Model Updater
- [x] `[P4]` `Architect/core/learning/preference_model_updater.py` — Create. Implement:
...
- [x] `[P4]` `tests/test_preference_model_updater.py` — Create. Must cover: load/update/validate/rollback logic; event emission.
...
- [x] `[P5]` `Architect/core/trajectory/trajectory_collector.py` — Create. Must implement:
...
- [x] `[P5]` `Architect/gui/views/trajectory_visualizer.py` — Create. Must implement:
...
- [x] `[P5]` `tests/test_trajectory_collector.py` — Create. Must cover: collection from event stream, trajectory file format, load logic.

### Alignment Tracking Dashboards
- [x] `[P5]` `Architect/core/alignment/alignment_tracker.py` — Create. Must implement:
...
- [x] `[P5]` `Architect/core/alignment/alignment_dashboard.py` — Create. Must implement:
...
- [x] `[P5]` `Architect/gui/views/alignment_dashboard.py` — Create. Must implement:
...
### Self-Model Insights
- [x] `[P5]` `Architect/core/self_model/self_model_engine.py` — Create. Must implement:
...
- [x] `[P5]` `Architect/core/self_model/insight_display.py` — Create. Must implement:
...
- [x] After each phase: Update `Architect/docs/PHASES.md` — change status (`Complete`/`In Progress`/`Not Started`), add completed deliverables, update blockers, update timeline.
- [x] After Phase 3: Update `Architect/docs/DIVERGENCES.md` — document event bus transport divergence (external APIs + NDJSON vs. file-based), archive divergence (JSON files vs. NDJSON events), protocol divergence (planned schema extension vs. 492-line spec).
- [x] After Phase 4: Update `Architect/docs/ARCHITECTURE.md` — describe RLHF integration data flow (`execution → outcome ingestion → feedback → model update → validation`), learning engine components (`skill_profiles`, `spaced_repetition`, `error_patterns`, `knowledge_base`, `risk_guards`, `define_eval`).
- [x] After Phase 5: Update `Architect/docs/ARCHITECTURE.md` — describe trajectory data flow (`event bus → trajectory collector → visualizer`), alignment tracking metrics, self-model insights.

### CI / Quality Gate Updates (Every Phase)
- [x] After Phase 2: Add `tests/test_memory_planner.py`, `tests/test_agent_assignment.py`, `tests/test_state_machine_transitions.py`, `tests/test_post_mortem_trigger.py`, `tests/test_baseline_retriever.py` to `Architect/Makefile` (`test` target: `python -m unittest discover -s tests -t .`).
- [x] After Phase 3: Add `tests/test_ouroboros.py`, `tests/test_event_bus.py`, `tests/test_manifest_parser.py`, `tests/test_manifest_validation.py`, `tests/test_archive.py`, `tests/test_timer.py` to `Makefile`.
- [x] After Phase 4: Add `tests/test_outcome_parser.py`, `tests/test_feedback_processor.py`, `tests/test_skill_profiles.py`, `tests/test_spaced_repetition.py`, `tests/test_error_patterns.py`, `tests/test_knowledge_base.py`, `tests/test_risk_guards.py`, `tests/test_define_eval.py` to `Makefile`.
- [x] After Phase 5: Add `tests/test_trajectory_collector.py`, `tests/test_alignment_tracker.py`, `tests/test_self_model_engine.py` to `Makefile`.

### Dependency & Environment Updates (Every Phase)
- [x] After Phase 2: Verify `Architect/requirements.txt` includes any new dependencies (`google-genai` for real GenAI calls; optional mock libraries for tests). If `google-genai` is already listed, confirm version compatibility.
- [x] After Phase 3: Verify `requirements.txt` includes `hdbscan` (if not already present; `Architect/limbs/ml/hdbscan_engine.py` imports it). If not present, add it.
- [x] After Phase 4: Verify `requirements.txt` includes `numpy` (already present for `MicrostructureEngine`, `ACGravityEngine`), `pydantic-settings` (already present), `scipy` (optional for SM-2 or statistical calculations; add if needed).
- [x] After Phase 5: Verify optional dependencies for visualization (`matplotlib` or `pyqtgraph`) documented but not added to `requirements.txt` by default (to keep core dependency model light; document in `docs/PHASES.md`).
...
### Phase 1 Lock
- [x] `ls Architect/docs/` shows `ARCHITECTURE.md`, `PHASES.md`, `DIVERGENCES.md`.
- [x] `cat Architect/README.md` shows directory map, installation, phase status, divergence notes.
- [x] `python Architect/test_init.py` exits `0` with 20+ assertions passing (no `AssertionError`).
- [x] `cat .gitignore` includes `runtime/`, `.env`, `workspace/`, `data/`.

### Phase 2 Completion
- [x] `python -m unittest discover -s tests -t .` passes all new Phase 2 tests (no `FAIL` or `ERROR`).
- [x] `python -c "from limbs.intelligence.genai_client import GenAIClient; c = GenAIClient(config=config); print('GenAI OK')"` runs without exception (stub mode acceptable for Phase 2 if SDK unavailable).
- [x] `Architect/runtime/plans/` exists (directory created by `MemoryPlanner`).
- [x] `Architect/core/state_machine.py` contains transition rules (search for `transition_map` or similar).
- [x] `tests/test_memory_planner.py` exists and passes.

### Phase 3 Completion
- [x] `tests/test_ouroboros.py` passes (`test_ouroboros_full_loop` exits `0`; `test_ouroboros_rollback_on_failure` exits `0`).
- [x] `tests/test_event_bus.py` passes (`VALID_EVENT_KINDS` enforced; NDJSON format verified).
- [x] `tests/test_manifest_validation.py` passes (`validate_manifest_update` returns `(True, ...)` for valid manifest; `(False, ...)` for invalid checksum or illegal file change).
- [x] `tests/test_timer.py` passes (deadline mode timeout triggered; unlimited mode `elapsed_s` increases; `safety_net_s` activation emits event; persistence works).
- [x] `tests/test_archive.py` passes (archive lookup, stats, verify, compact all work).
- [x] `Architect/runtime/system.log` exists (created by event bus emission in test or manual run).
- [x] `Architect/runtime/backups/` exists (created by `SelfModificationSafety` backup logic).

### Phase 4 Completion
- [x] `tests/test_define_eval.py` passes (`define_eval` interface works; `EvalPipeline.run()` returns valid `EvalResult`).
- [x] `tests/test_feedback_dialog.py` passes (submit logic works; file saved in correct format).
- [x] `tests/test_preference_model_updater.py` passes (update, validate, rollback all work; events emitted correctly).
- [x] `tests/test_spaced_repetition.py` passes (SM-2 intervals computed correctly; review schedule saved).
- [x] `tests/test_skill_profiles.py` passes (Wilson score computed; persistence works).
- [x] `tests/test_error_patterns.py` passes (patterns detected; alerts emitted for threshold exceeded).
- [x] `tests/test_knowledge_base.py` passes (knowledge entries saved; search works; extraction from reflection output works).
- [x] `tests/test_risk_guards.py` passes (risk assessments match `TheJudgeM8` invariants; events emitted for failures/warnings).

### Phase 5 Completion
- [x] `tests/test_trajectory_collector.py` passes (trajectory collected from event stream; file format `jsonl`; load logic works).
- [x] `tests/test_alignment_tracker.py` passes (alignment metrics computed from trajectory + feedback + risk guards; results saved in correct format).
- [x] `Architect/gui/app.py` updated (new `AlignmentDashboardTab` added to `GMTMainWindow`; `TrajectoryVisualizer` or `AlignmentDashboard` referenced or added).
- [x] `tests/test_self_model_engine.py` passes (insights generated; saved in correct format; format for display works).
