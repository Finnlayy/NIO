...
## 2026-09-19 - Keyboard Event Collision: Modal Dialogs and Centralized Hotkeys
**Learning:** A keyboard event collision pattern was discovered where local 'Escape' keydown listeners inside modal components (`GalleryDrawer`, `McpConsole`) were indiscriminately intercepting keystrokes. This bypassed the `isTypingTarget` input guards managed by the centralized `opsHotkey` global handler, leading to functional regressions when attempting to close panels while focus was inside text inputs.
**Action:** Always provide `role="dialog"`, `aria-modal="true"`, and an `autoFocus` target within modal components to guarantee immediate keyboard traversability. Do not add redundant local `keydown` event listeners for 'Escape' inside modals if a centralized, guarded global hotkey handler (like `opsHotkey` with `isTypingTarget`) already exists.
