...
## 2026-09-09 - React useMemo Overhead for Static Derivations
**Learning:** Initializing derived properties using `useMemo` based on static module imports (like `networkNodes`) inside a Next.js frontend component (`NetworkDashboard`) creates unnecessary overhead. The hook executes on the first render and React must allocate memory to track its dependency array, even though the data source is a frozen static object that never changes during the application lifecycle.
**Action:** Lift derived properties that depend exclusively on static configuration files (like `NODE_BY_ID`, `ACTIVE_DOMAIN_COUNT`, `AGENT_COUNT`) completely out of the React component to the module level. This guarantees zero render loop cost and no hook tracking overhead for constant data.
