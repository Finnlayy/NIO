...
## 2026-09-09 - Accessible Modals and Esc Handler Collisions
**Learning:** Adding local 'Escape' event listeners inside React modals bypasses global hotkey input suppression (`isTypingTarget`), indiscriminately stealing keystrokes (like pressing Esc to close a dropdown inside a text input). Additionally, modals lacking native ARIA dialogue semantics (`role="dialog"`, `aria-modal="true"`) and an explicit `autoFocus` target break keyboard traversability when opened.
**Action:** Remove local `keydown` handlers for 'Escape' in modals if a centralized global handler already exists. Add `role="dialog"`, `aria-modal="true"`, and an explicit `aria-label` to the modal container. Apply `autoFocus` to a safe initial element (like the close button) to guarantee immediate keyboard focus upon opening.
