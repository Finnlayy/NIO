...
## 2026-09-09 - Accessible Modal Shortcuts and Focus Traps
**Learning:** Redundant local `Escape` keydown listeners inside modals bypass global input guards (e.g. `isTypingTarget`), indiscriminately stealing keystrokes from user inputs. Furthermore, modals without `autoFocus` on an initial element fail to capture keyboard traversability, requiring users to click to engage with the modal.
**Action:** Remove local `Escape` listeners inside modals and rely strictly on the centralized hotkey dispatcher (`opsHotkey` evaluating `close-panels`) to handle dismissal deterministically. Always assign `role="dialog"`, `aria-modal="true"`, and `autoFocus` to an internal element (like a close button) to guarantee immediate keyboard access and semantic correctness.
