...
## 2024-05-14 - [Graphics / Rendering Insight]
**Learning:** Using `width`, `marginLeft`, and `transition-all` triggers heavy CPU layout-thrashing and degrades framerate.
**Action:** Enforced hardware-accelerated transitions by replacing `transition-all` with `transition-colors` and replacing width animations with `transform: scaleX(...)` and `transformOrigin`.
