...
def _listener_alive(path: Path) -> bool:
    ...
    Ein ``sendto`` auf einem UDS-Datagramm-Socket autocnectet im Kernel und
    antwortet mit ``ECONNREFUSED``, wenn gar kein Socket mehr gebunden ist. Das
    ist der billige Test -- kein Roundtrip, kein Timeout, den man abwarten muss.
    """
    probe = socket.socket(socket.AF_UNIX, socket.SOCK_DGRAM)
    try:
        probe.sendto(b"", str(path))
    except OSError:
        return False
    finally:
        probe.close()
