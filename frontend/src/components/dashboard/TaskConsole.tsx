...
export function TaskConsole({
  ...
  return (
    <Tile className="p-4 h-full flex flex-col">
      ...
      <div
        className={isRunning || !taskText.trim() ? "cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60 rounded-lg" : ""}
        title={
          isRunning || !taskText.trim()
            ? "Task description is required and core must be idle"
            : undefined
        }
        tabIndex={isRunning || !taskText.trim() ? 0 : undefined}
      >
        <button
          onClick={onRun}
          disabled={isRunning || !taskText.trim()}
          className="primary-button w-full mt-3 flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-cyan-400/60 focus-visible:outline-none"
        >
