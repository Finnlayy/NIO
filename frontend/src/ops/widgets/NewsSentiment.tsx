...
export function NewsSentiment({
  ...
  const positive =
    source === "live"
      ...
      : (data.positive as number);
  const negative =
    source === "live"
      ? Math.round((negCount / total) * 100)
      : (data.negative as number);
  const neutral =
    source === "live" ? 100 - positive - negative : (data.neutral as number);
  ...
  return (
    <div className="flex flex-col h-full">
      <p
        className={`text-xl font-bold flex items-center gap-2 ${verdictColor}`}
      >
        {negative > positive + 10
          ? "Mostly negative"
          : positive > negative + 10
      ...
      <p className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1.5 text-[10px] text-slate-500">
        ...
        <span className="flex items-center gap-1 font-mono tabular-nums">
          <span className="w-1.5 h-1.5 rounded-full bg-slate-500" /> Neutral{" "}
          {neutral}%
        </span>
        <span className="flex items-center gap-1 font-mono tabular-nums">
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-slate-600 mt-1 flex items-center gap-1.5 font-mono tabular-nums">
                ...
                {item.highImpact && (
                  <span className="px-1 rounded bg-rose-500/15 text-rose-300 font-semibold">
                    HIGH IMPACT
                  </span>
                )}
              </p>
            ...
            <span
              className={`text-[10px] font-bold shrink-0 mt-0.5 ${sentColor[item.sentiment]}`}
            >
              {item.sentiment}
            </span>
          </div>
