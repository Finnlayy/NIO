    ...
    >
      <svg viewBox="0 0 24 16" className="w-6 h-4">
        <path
          d={
            bullish
              ? "M2 14 L6 6 L12 9 L18 3 L20 5 L13 11 L7 8 Z"
              : "M2 2 L6 10 L12 7 L18 13 L20 11 L13 5 L7 8 Z"
          }
          fill="none"
  ...
  return (
    <div className="flex flex-col h-full">
      <p className="text-lg font-bold text-white">
        {patterns.length} forming, none broken yet
      </p>

      {/* bull/bear progress */}
      <div className="flex h-1.5 rounded-full overflow-hidden mt-2.5">
        <div
          style={{ width: `${(bullish / total) * 100}%`, background: upBar }}
        />
        <div
          style={{ width: `${(bearish / total) * 100}%`, background: downBar }}
        />
      </div>
      ...
      <div className="space-y-0">
        {patterns.map((p) => (
          ...
          >
            ...
            <span className="w-14 text-right text-[10px] text-slate-500 shrink-0">
              {p.atrStatus}
            </span>
          </div>
        ))}
