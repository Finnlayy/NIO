  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-4 gap-2 mt-3 text-center">
        ...
        <Stat
          label="Risk"
          value={pct(plan.riskPct)}
          valueClass="text-rose-400"
        />
        <Stat
          label="Reward (T1)"
          value={pct(plan.rewardPct)}
      ...
      <p className="text-[10px] text-slate-600 mt-2">
        Blueprint for {symbol ?? "—"} · simulated levels
      </p>
    </div>
  );
}
  ...
  return (
    <div className="absolute inset-x-0 flex items-center" style={{ top: y }}>
      ...
      <span
        ...
        style={{ color, borderColor: `${color}55`, borderWidth: 1 }}
      >
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[11px] font-mono tabular-nums ${color} w-14 text-right`}
      >
        {pctValue === 0
          ? "—"
...
function Stat({
  ...
  return (
    <div>
      ...
      <p
        className={`font-mono tabular-nums font-bold mt-0.5 ${small ? "text-[11px]" : "text-[13px]"} ${valueClass ?? "text-slate-100"}`}
      >
        {value}
      </p>
    </div>
