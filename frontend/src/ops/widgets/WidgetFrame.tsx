...
import { dispatchRefresh, dispatchRemove, useGridStore } from "../store";
 ...
 */
export const WidgetFrameContent = memo(function WidgetFrameContent({
  widget,
}: {
  widget: WidgetInstance;
}) {
      ...
      <header className="flex items-center gap-2 px-3.5 h-11 border-b border-white/[0.06] shrink-0">
        {!widget.pinned ? (
          ...
          <span
            ...
            title="Pinned — reordering locked"
          >
        ...
        <span className="ml-auto flex items-center gap-0.5 shrink-0 relative">
          <span
            className="w-1.5 h-1.5 rounded-full bg-emerald-400 pulse-dot mr-1"
            title="Live data"
          />
          ...
          >
            <Minus
              className={`w-3.5 h-3.5 transition-transform ${widget.minimized ? "rotate-180" : ""}`}
            />
          </IconBtn>
          <IconBtn
            ...
            {menuOpen && (
              <>
                ...
                >
                  ...
                  <MenuItem
                    danger
                    onClick={() => dispatchRemove(widget.id)}
