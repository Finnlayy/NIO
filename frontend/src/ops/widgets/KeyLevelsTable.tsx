            ...
            <div key={`${lvl.label}-${i}`} className="flex items-center gap-2 text-[11px]">
              ...
              <span className="flex-1 h-[6px] relative">
                {isNow ? (
                  ...
                  <span
                    className="absolute top-1/2 -translate-y-1/2 rounded-[2px] border w-full"
                    style={{
                      clipPath: `inset(0% ${Math.max(0, 100 - Math.max(10, lvl.width * 100) - (lvl.distancePct < 0 ? Math.max(0, (1 - lvl.width) * 100 - 40) : 0))}% 0% ${lvl.distancePct < 0 ? Math.max(0, (1 - lvl.width) * 100 - 40) : 0}%)`,
                      borderColor: color,
                      background: `${color}22`,
                      height: "8px",
                    }}
