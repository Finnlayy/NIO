...
function gradientIds(html) {
  return (html.match(/<linearGradient id="([^"]*)"/g) ?? []).map(
    (s) => s.match(/id="([^"]*)"/)[1],
  );
}

function urlRefs(html) {
  return (html.match(/url\(#([^)]*)\)/g) ?? []).map((s) =>
...
test("gradient id is stable across re-renders (paint reference preserved)", () => {
  ...
  const first = collectByType(
    m.render(() => Sparkline({ values })),
    "linearGradient",
  );
  const second = collectByType(
    m.render(() => Sparkline({ values })),
    "linearGradient",
  );
  ...
  assert.equal(
    second[0].props.id,
    first[0].props.id,
    "id must not churn on re-render",
  );
});
...
test("sparkline renders polyline + gradient fill with the given color", () => {
  const values = [0.2, 0.4, 0.3, 0.7, 0.6];
  const html = renderToStaticMarkup(
    React.createElement(Sparkline, { values, color: downBar, height: 170 }),
  );
  const id = gradientIds(html)[0];
  assert.ok(id, "gradient defined");
  assert.ok(
    html.includes(`fill="url(#${id})"`),
    "polygon fill references own gradient",
  );
  assert.ok(
    html.includes(`stroke="${downBar}"`),
    "stroke uses the given color",
  );
  const pts = (html.match(/<polyline points="([^"]*)"/) ?? [])[1] ?? "";
  assert.equal(
    pts.trim().split(" ").length,
    values.length,
    "one point per value",
  );
});
...
test("integration: two Envelope Charts on the page do not leak gradient ids", () => {
  ...
  assert.notEqual(
    ids[0],
    ids[1],
    "widget instances must not share gradient ids",
  );
  resetStore();
});
