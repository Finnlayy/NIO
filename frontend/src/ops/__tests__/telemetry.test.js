...
const MICRO = {
  imbalance_ratio: 0.24,
  depth_2pct: 0.61,
  footprint_delta: [0.1, -0.2, 0.3],
};
const GRAVITY = {
  l2_depth: 0.61,
  l3_iceberg: 0.18,
...
test("wire contract: a missing field is rejected, not defaulted", () => {
  ...
  assert.equal(
    parseTelemetryRecord(record("microstructure_tick", noVector)),
    null,
  );
});
...
test("wire contract: NaN, Infinity, strings and booleans are rejected", () => {
  for (const bad of [Number.NaN, Number.POSITIVE_INFINITY, "0.5", true, null]) {
    assert.equal(
      parseTelemetryRecord(
        record("gravity_tick", { ...GRAVITY, v_total: bad }),
      ),
      null,
  ...
  assert.equal(
    parseTelemetryRecord(
      record("microstructure_tick", { ...MICRO, footprint_delta: [] }),
    ),
    null,
  ...
  assert.equal(
    parseTelemetryRecord(
      record("microstructure_tick", { ...MICRO, footprint_delta: [0.1, "x"] }),
    ),
    null,
  );
});
...
test("wire contract: unknown kinds and non-objects are rejected", () => {
  assert.equal(
    parseTelemetryRecord({ kind: "execution_complete", payload: GRAVITY }),
    null,
  );
  assert.equal(parseTelemetryRecord(null), null);
  assert.equal(parseTelemetryRecord("nope"), null);
  assert.equal(
    parseTelemetryRecord({ kind: "gravity_tick" }),
    null,
    "missing payload",
  );
});
...
test("fold keeps the other kinds and stamps the arrival time", () => {
  ...
  assert.deepEqual(
    snap.gravity,
    GRAVITY,
    "an earlier kind must survive a later one",
  );
...
test("state machine: no transport means DISCONNECTED, whatever arrived before", () => {
  ...
  assert.equal(
    nextFeedConnection(
      "CONNECTED_LIVE",
      { transportOpen: false, lastTickAt: at },
      at,
    ),
    "DISCONNECTED",
  );
...
test("state machine: a tick inside the window is CONNECTED_LIVE", () => {
  ...
  assert.equal(
    nextFeedConnection(
      "DISCONNECTED",
      { transportOpen: true, lastTickAt: at - 100 },
      at,
    ),
    "CONNECTED_LIVE",
  );
...
test("state machine: silence beyond 3000 ms degrades to STALE_CACHE_DEGRADED", () => {
  assert.equal(
    STALE_AFTER_MS,
    3000,
    "must match the brief and the server-side STALE_AFTER_MS",
  );
  const last = 10_000;
  assert.equal(
    nextFeedConnection(
      "CONNECTED_LIVE",
      { transportOpen: true, lastTickAt: last },
      last + STALE_AFTER_MS,
    ),
    "CONNECTED_LIVE",
...
test("state machine: an open transport with no tick yet keeps its state", () => {
  ...
  assert.equal(
    nextFeedConnection(
      "DISCONNECTED",
      { transportOpen: true, lastTickAt: 0 },
      5000,
    ),
    "DISCONNECTED",
  );
...
test("CvdHeatmap fails closed: without a live tick it shows the template and says so", () => {
  ...
  const html = renderToStaticMarkup(
    m.render(() => CvdHeatmap({ data: template.dataFactory() })),
  );
  ...
  assert.match(
    html,
    /nicht der Markt/,
    "must not present the mock as market data",
  );
});
