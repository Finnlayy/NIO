...
export function adaptOhlcv(raw: Raw): LiveOhlcv | null {
  const bars = asArray(
    raw?.bars ?? raw?.ohlcv ?? raw?.candles ?? raw?.data ?? raw,
  );
  const closes = bars
    .map((b) => num(findField(b, ["close", "c", "closePrice", "price"])))
...
export function adaptTechnicals(raw: Raw): LiveTechnical | null {
  ...
  const score = (() => {
    const s = num(
      findField(t, ["score", "confluence", "confluence_score", "rating_score"]),
    );
    if (s !== null && s >= 0 && s <= 100) return Math.round(s);
    const idx = VERDICT_ORDER.findIndex((v) => rating.includes(v));
    return idx >= 0
      ? Math.round(((idx + 0.5) / VERDICT_ORDER.length) * 100)
      : 50;
  })();
  ...
  const rsi =
    num(findField(ind, ["rsi", "RSI", "rsi14"])) ??
    num(findField(t, ["rsi"])) ??
    50;
...
export function adaptNews(raw: Raw): LiveNews[] | null {
  const items = asArray(
    raw?.news ?? raw?.stories ?? raw?.articles ?? raw?.data ?? raw,
  );
  const out: LiveNews[] = items.slice(0, 8).map((n) => {
    const sentiment =
      str(findField(n, ["sentiment", "sentimentLabel", "tone"]))
        ?.toUpperCase()
        .slice(0, 3) ?? "NEU";
    const published =
      str(findField(n, ["published", "time", "date", "publishedAt"])) ?? "";
    return {
      headline:
      ...
      sentiment: ["POS", "NEG", "NEU"].includes(sentiment)
        ...
        : "NEU",
      highImpact:
        Boolean(findField(n, ["highImpact", "important", "breaking"])) || false,
    };
  });
...
export function adaptScreener(raw: Raw): LiveRank[] | null {
  ...
  const out: LiveRank[] = rows
    .map((r, i) => {
      const score = num(
        findField(r, ["compositeScore", "score", "rank_score", "setupScore"]),
      );
      if (score === null) return null;
      return {
        rank: i + 1,
        symbol:
          str(findField(r, ["symbol", "ticker", "name"]))
            ?.split(":")
            .pop() ?? `SYM${i}`,
        ...
        wkChange:
          num(
            findField(r, ["change_1w", "week", "perf_week", "WK", "perf_w"]),
          ) ?? 0,
        moChange:
          num(
            findField(r, ["change_1m", "month", "perf_month", "MO", "perf_m"]),
          ) ?? 0,
        technicals:
          num(findField(r, ["technicals", "tech_score", "recommend_score"])) ??
          0.5,
        volume: "5/20",
      };
    })
